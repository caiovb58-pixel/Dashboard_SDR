...
export default function LeaderPortal({
  ...
  const teamConversionRate = teamTotalAgendamentos > 0 
    ...
    : 0;
  const activeMonth = monthAndThermometer?.month || new Date().toISOString().slice(0, 7);
  const teamThermometer = calculateGoalProgress(teamSDRs, activeMonth);
      ...
      const response = await fetch('/api/gemini/guidance', {
        ...
        body: JSON.stringify({
          ...
          sdrStats: teamSDRs.map(s => {
            const progress = calculateGoalProgress([s], activeMonth);
            return {
              name: s.name,
              ...
              expectedAgendamentosToday: progress.expectedRealizedToday,
              progressGap: progress.progressGap,
              active: s.active,
              team: s.team
            };
          }),
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-4 shadow-2xs">
          <span className="block text-[9px] font-bold text-neutral-400 uppercase tracking-widest">Conversão Média Célula</span>
          <div className="mt-1.5 flex items-baseline gap-1">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-4 shadow-2xs flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <span className={`inline-flex text-[9px] font-black uppercase px-2 py-0.5 rounded border ${teamThermometer.labelColor}`}>
            {teamThermometer.temperature}
          </span>
          <p className="text-xs text-neutral-600 mt-2">
            A celula esta em <strong className="text-black">{teamThermometer.realizedProgress}%</strong> da meta de agendamentos, contra <strong className="text-black">{teamThermometer.expectedProgress}%</strong> esperado para o dia {teamThermometer.currentDaysElapsed}. Faltam <strong className="text-black">{teamThermometer.remainingToGoal}</strong> agendamentos.
          </p>
        </div>
        <div className="w-full lg:max-w-sm">
          <div className="flex justify-between text-[10px] font-mono font-bold text-neutral-500 mb-1">
            <span>Ritmo atual: {teamThermometer.currentDailyPace}/dia</span>
            <span>Necessario: {teamThermometer.requiredDailyPace}/dia</span>
          </div>
          <div className="relative h-3 bg-neutral-100 border border-neutral-200 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${teamThermometer.barColor}`}
              style={{ width: `${Math.min(100, teamThermometer.realizedProgress)}%` }}
            ></div>
            <div
              className="absolute top-0 bottom-0 w-0.5 bg-red-500"
              style={{ left: `${Math.min(99, teamThermometer.expectedProgress)}%` }}
            ></div>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
                {teamSDRs.length === 0 ? (
                  ...
                  teamSDRs.map(sdr => {
                    ...
                    const sdrProgress = calculateGoalProgress([sdr], activeMonth);
                    const isBelowGoals = sdr.agendamentosCount < sdrProgress.expectedRealizedToday || sdrRate < (sdr.metaEfetivacaoRate || 50);
